...
#define MENU_LINE_H 20 // 行高 (适配Adafruit 6x8字体)
...
int menu_command_callback(enum _menu_command command, ...)
{
    ...
    case SHOW_STRING: // 参数:( int x, int y, char *str ); 返回: uint8_t 字符串长度;
    {
        ...
        display.setCursor(show_x, show_y);
        display.print(show_string);
    ...
    {
        retval = Encoder_Sw_Down();
    }
    break;

    case GET_EVENT_BACK:
    {
        retval = Encoder_Sw_LongPress();
    }
    break;

    case GET_EVENT_WHEEL:
    {
        retval = Encoder_Read_Dir();
    }
    break;
...
void MENU_ShowOptionList(MENU_HandleTypeDef *hMENU)
{
    ...
    for (int16_t i = -1; i <= CURSOR_CEILING + 1; i++) // 遍历显示 选项
    {
        ...
        int16_t x;
        if (hMENU->IsSettingList)
        {
            x = MENU_X + MENU_MARGIN + MENU_PADDING; // 左对齐
        }
        else
        {
            x = MENU_X + ((MENU_WIDTH - (hMENU->OptionList[hMENU->Show_i + i].StrLen * MENU_FONT_W)) / 2); // 居中
        }
...
void MENU_ShowCursor(MENU_HandleTypeDef *hMENU)
{
    ...
    if (hMENU->AnimationUpdateEvent)
    {
        ...
        // 根据IsSettingList决定光标左对齐还是居中
        if (hMENU->IsSettingList)
        {
            target_xsta = MENU_X + MENU_MARGIN + MENU_PADDING;
        }
        else
        {
